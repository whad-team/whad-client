"""
WHAD Enhanced ShockBurst base connector
...
from whad.hub.esb import ESBMetadata, EsbNodeAddress, Commands
...
class ESB(Connector):
    ...
    This connector drives a Enhanced ShockBurst (ESB) capable device with
    ESB-specific WHAD messages. It is required by various role classes to
    interact with a real device and pre-process domain-specific messages.
    """
    ...
    def close(self):
        """Stop the WHAD adpater and close its related device.
        """
        self.stop()
        self.device.close()
    ...
    def can_send(self):
        ...
        if self.__can_send is None:
            commands = self.device.get_domain_commands(Domain.Esb)
            self.__can_send = ((commands & (1 << Commands.Send))>0 or
                               (commands & (1 << Commands.SendRaw)))
    ...
    def on_raw_pdu(self, packet: Packet):
        """Callback to handle incoming raw PDU

        :param packet: Incoming PDU
        :type packet: Packet
        """
    ...
    def on_pdu(self, packet):
        """Callback for incoming PDU.

        For now, all compatible devices report raw PDUs when it comes to ESB,
