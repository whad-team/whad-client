    ...
    def inject(self, packet):
        ...
        if not self._metadata_check:
            ...
            if deviation is not None:
                self.stop()
                if hasattr(packet, "metadata") and hasattr(packet.metadata, "modulation"):
                    if packet.metadata.modulation == PhyModulation.ASK:
                        self.set_ask()
                    elif packet.metadata.modulation == PhyModulation.FSK and deviation is not None:
                        ...
                        self.set_qpsk()
                    elif packet.metadata.modulation == PhyModulation.LORA:
                        self.set_lora(
                            self.__configuration.lora_configuration.spreading_factor,
                            self.__configuration.lora_configuration.coding_rate,
                            self.__configuration.lora_configuration.bandwidth,
                ...
                if hasattr(packet, "metadata") and hasattr(packet.metadata, "frequency"):
                    self.set_frequency(packet.metadata.frequency)

                if hasattr(packet, "metadata") and hasattr(packet.metadata, "packet_size"):
                    self.set_packet_size(packet.metadata.packet_size)

                if hasattr(packet, "metadata") and hasattr(packet.metadata, "datarate"):
                    self.set_datarate(packet.metadata.datarate)

                if hasattr(packet, "metadata") and hasattr(packet.metadata, "syncword"):
