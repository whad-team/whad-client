...
class BLE(WhadDeviceConnector):
    ...
    def can_scan(self):
        ...
        commands = self.device.get_domain_commands(Domain.BtLE)
        return (
            (commands & (1 << Commands.ScanMode)) > 0
            and (commands & (1 << Commands.Start)) > 0
            and (commands & (1 << Commands.Stop)) > 0
        )
    ...
    def can_be_central(self):
        ...
        commands = self.device.get_domain_commands(Domain.BtLE)
        return (
            (commands & (1 << Commands.CentralMode)) > 0
            and (commands & (1 << Commands.Start)) > 0
            and (commands & (1 << Commands.Stop)) > 0
        )
    ...
    def can_be_peripheral(self):
        ...
        commands = self.device.get_domain_commands(Domain.BtLE)
        return (
            (commands & (1 << Commands.PeripheralMode)) > 0
            and (commands & (1 << Commands.Start)) > 0
            and (commands & (1 << Commands.Stop)) > 0
        )
    ...
    def can_discover_access_addresses(self):
        ...
        commands = self.device.get_domain_commands(Domain.BtLE)
        return (
            (commands & (1 << Commands.SniffAccessAddress)) > 0
            and (commands & (1 << Commands.Start)) > 0
            and (commands & (1 << Commands.Stop)) > 0
        )
    ...
    def can_sniff_active_connection(self):
        ...
        commands = self.device.get_domain_commands(Domain.BtLE)
        return (
            (commands & (1 << Commands.SniffActiveConn)) > 0
            and (commands & (1 << Commands.Start)) > 0
            and (commands & (1 << Commands.Stop)) > 0
        )
    ...
    def can_sniff_advertisements(self):
        ...
        commands = self.device.get_domain_commands(Domain.BtLE)
        return (
            (commands & (1 << Commands.SniffAdv)) > 0
            and (commands & (1 << Commands.Start)) > 0
            and (commands & (1 << Commands.Stop)) > 0
        )
    ...
    def can_sniff_new_connection(self):
        ...
        commands = self.device.get_domain_commands(Domain.BtLE)
        return (
            (commands & (1 << Commands.SniffConnReq)) > 0
            and (commands & (1 << Commands.Start)) > 0
            and (commands & (1 << Commands.Stop)) > 0
        )
    ...
    def prepare(
        self, *packets, trigger=ManualTrigger(), direction=Direction.MASTER_TO_SLAVE
    ):
        """
        ...
        if isinstance(trigger, ManualTrigger):
            ...
            msg = self.hub.ble.create_prepare_sequence_manual(
                trigger.identifier, direction, prep_packets
            )
            ...
            msg = self.hub.ble.create_prepare_sequence_conn_evt(
                trigger.identifier, direction, trigger.connection_event, prep_packets
            )
    ...
    def sniff_new_connection(
        self,
        channel=37,
        show_advertisements=True,
        show_empty_packets=False,
        bd_address="FF:FF:FF:FF:FF:FF",
    ):
        """
    ...
    def enable_peripheral_mode(self, adv_data: bytes = None, scan_data: bytes = None):
        ...
        msg = self.hub.ble.create_periph_mode(adv_data=adv_data, scan_rsp=scan_data)
    ...
    def on_connected(self, connection_data: dict):
        ...
        logger.debug(
            "connection handle: %d" % connection_data.conn_handle
            if connection_data.conn_handle is not None
            else 0
        )
    ...
    def send_pdu(
        ...
        if self.can_send():
            ...
            if encrypt is not None and isinstance(encrypt, bool):
                ...
                logger.info(
                    "[ble connector] link-layer encryption: %s" % self.__encrypted
                )
