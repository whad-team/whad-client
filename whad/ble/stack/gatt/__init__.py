...
class GattClientServer(GattClient):
    """Provides a GATT client implementation that implements a set of GATT
    client procedures, but also exposes a GATT profile (set of services,
    characteristics and descriptors) as defined in the provided profile.

    The usual GATT server procedures are supported and any connected peripheral
    can interact with this server as well.
    """
...
class GattServerClient(GattServer):
    """Provides a GATT server implementation that reacts to sollicitations from
    a remote GATT client (as generally used in BLE peripherals) while supporting
    a set of client-oriented GATT procedures that can be initiated from our
    Python code, like a classic GATT client would do.
    """

    def __init__(self, parent=None, layer_name=None, options=None):
        """Initialize our GATT Server+Client layer
        """
        super().__init__(parent=parent, layer_name=layer_name,
                         options=options if options is not None else {})

        # Client GATT model and notification callbacks
        self.__model = None
        self.__notification_callbacks = {}

    @property
    def model(self) -> GenericProfile:
        """Remote server GATT model
        """
        return self.__model
    ...
    def set_client_model(self, model):
        '''This method is used with GattServerClient to specify the GATT
        client device profile.
        '''
        if isinstance(model, GenericProfile):
            self.__model = model
    ...
    ######################################
    # Supported Client response handlers
    ######################################
    ...
    @txlock
    def on_handle_value_indication(self, indication):
        """ATT Handle Value Indication

        :param int handle: Attribute handle
        :param value: Attribute value
        """
        if indication.handle in self.__notification_callbacks:
            self.__notification_callbacks[indication.handle](
                indication.handle,
                indication.value,
                indication=True
            )
    ...
    #####
    # Extra client-oriented GATT procedures
    #####
    ...
    def register_notification_callback(self, handle, cb):
        """Register a notification callback associated with a specific handle.
    ...
    def unregister_notification_callback(self, handle):
        """Unregister a notification callback previously associated with a
        specific handle.
    ...
    def discover(self, save_values: bool = False):
        """Discover our remote peer's GATT services, characteristics and
        descriptors.

        :param save_values: If enabled, attributes' values are read and saved in the model
