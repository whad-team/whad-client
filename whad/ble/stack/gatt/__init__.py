...
class GattClientServer(GattClient, GattServer):
    """Provides a GATT client implementation that offers a set of GATT
    ...
    def __init__(self, parent=None, layer_name=None, options=None):
        """Gatt client initialization.
        """
        super().__init__(parent=parent, layer_name=layer_name, options=options)

class GattServerClient(GattServer, GattClient):
    """Provides a GATT server implementation that supports a set of GATT
    server procedures but also some extra client-oriented procedures to interact
    with a possible GATT server exposed by the connected peer.
    """
